...
pc_obj <- subset(Tfh_obj, 
                 subset = Tfh_clone_id != 'other' & #subset the picked clonal lineages
                   MKI67 == 0) # remove cycling cells
...
saveRDS(pc_obj, './10x/objects/TwoYear_Donor_321-05_pickedClones.rds')
write_csv(pc_markers, './10x/outs/TwoYear_Donor_321-05_pickedClones_markers.csv')
...
ggsave('./10x/outs/TwoYear_Donor_321-05_Tfh_pickedClone_umaps.png', plot = pc_umap_panel,
       height = 10, width = 15)
...
ggsave('./10x/outs/TwoYear_Donor_321-05_Tfh_pickedClone_pca_time.png', plot = long_pca,
       height = 4, width = 18)
...
ggsave('./10x/outs/TwoYear_Donor_321-05_Tfh_pickedClone_fancy_pca_time.pdf', 
       plot = fancy_pca,
...
picked_degs_scale_data <- scale_data[picked_tfh_gene_set, ] %>% 
  as.data.frame() %>%
...
ggsave('./10x/outs/TwoYear_Donor_321-05_Tfh_pickedClone_picked_marker_hm.pdf', plot = pc_hm,
...
ggsave('./10x/outs/TwoYear_Donor_321-05_Tfh_pickedClone_PC_marker_hm.pdf', plot = pc_hm2,
       height = 8, width = 13, useDingbats = F)
...
ggsave('./10x/outs/TwoYear_Donor_321-05_Tfh_pickedClones_lineages_picked_markers_hm.pdf', plot = lineage_hm_lay,
       height = 12.5, width = 17, useDingbats = F)
PC_lineage_hm_lay <- wrap_plots(PC_lineage_hm) 
ggsave('./10x/outs/TwoYear_Donor_321-05_Tfh_pickedClones_lineages_PC_markers_hm.pdf', plot = PC_lineage_hm_lay,
       height = 12.5, width = 17, useDingbats = F)
...
FNA_df$time <- factor(FNA_df$time, levels = names(TimePal2))

bloodpal <- c(TissuePal[2], 'black')
names(bloodpal) <- c('yes', 'no')

# alluvial plots
picked_pl_FNA <- ggplot(data = FNA_df,
                        aes(x = time, 
                            y = freq, 
...
ggsave('./10x/outs/TwoYear_BothDonors_321-05_pickedClone_clone_freqs.pdf', 
       plot = picked_pl_FNA,
...
ggsave('./10x/outs/TwoYear_BothDonors_321-05_pickedClone_clone_number.pdf', 
...
ggsave('./10x/outs/TwoYear_BothDonors_321-05_pickedClone_flu_specific_clone_freqs.pdf', 
       plot = picked_pl_hits,
       height = 6, width = 12, useDingbats = F)
...
for (i in seq_along(bulk_d5_chains)){
  ...
  filename <- paste0('./10x/outs/TwoYear_BothDonors_321-05_pickedClone_bulk_', chain,'.pdf')
  ggsave(filename, 
         plot = alluv_pl,
...
pickedClone_alluvials <- (picked_pl_FNA_n / alluv_plots[[2]]) + plot_layout(guides = 'collect')

ggsave('./10x/outs/TwoYear_BothDonors_321-05_pickedClone_10x_and_bulk_TCRb.pdf', 
       plot = pickedClone_alluvials,
       height = 9, width = 8, useDingbats = F)
...
pc_hm_long <- pheatmap(degs_scale_data_t, 
  ...
  as.ggplot(.)
ggsave('./10x/outs/TwoYear_Donor_321-05_Tfh_pickedClone_gex_long.pdf', plot = pc_hm_long,
...
ggsave('./10x/outs/TwoYear_Donor_321-05_Tfh_pickedClone_gex_averaged.pdf', 
       plot = pc_hm_avg,
       height = 8, width = 13, useDingbats = F)