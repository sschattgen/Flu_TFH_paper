...
IL10_PM <- read.delim('./10x/outs/TwoYear_Tfh_lineages_IL10_TFH_vs_premem_markers.tsv') %>%
  arrange(desc(avg_log2FC))
IL10_GC <- read.delim('./10x/outs/TwoYear_Tfh_lineages_IL10_TFH_vs_GC_markers.tsv')%>%
  arrange(desc(avg_log2FC))
EM_GC <- read.delim('./10x/outs/TwoYear_Tfh_lineages_premem_vs_GC_markers.tsv')%>%
  arrange(desc(avg_log2FC))
...
pc_hm <- pheatmap(mod_mat, 
  ...
  as.ggplot(.)
ggsave('./10x/outs/TwoYear_Donor_321-05_Tfh_pickedClone_modules.pdf', plot = pc_hm,
...
ggsave('./10x/outs/TwoYear_Donor_321-05_Tfh_pickedClones_lineages_module_hm.pdf', plot = lineage_hm_lay,
