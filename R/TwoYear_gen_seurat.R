...
gexFiles <- Read10X('./10x/objects/JCC280_Year2Aggreg_CR6/filtered_feature_bc_matrix/')
...
md <- read.csv('./scripts/final_scripts/library_tissue_map.csv')
...
# read metadata
tcrmeta <- read.csv('./10x/objects/tcr_metadata.csv', stringsAsFactors = F)
...
map_files <- './10x/objects/TwoYear_TCR_clones.tsv.barcode_mapping.tsv'
...
clone_files <- './10x/objects/TwoYear_TCR_clones.tsv'
...
# read metadata
bcrmeta <- read.csv('./10x/objects/bcr_metadata.csv', stringsAsFactors = F)

#read in mappings
#barcode suffixes in the map_files.list need to be adjusted to match each corresponding library in the aggreagated gex dataset to match metadata
map_files <- './10x/objects/TwoYear_BCR_clones.tsv.barcode_mapping.tsv'
map_files.list <- map( map_files, ~ read.delim( . , stringsAsFactors = F) )
names(map_files.list) <- 'x' 

#read in clone files and add subjects to df
clone_files <- './10x/objects/TwoYear_BCR_clones.tsv'
clone_files.list <- map( clone_files, ~ read.delim( . , stringsAsFactors = F) )
names(clone_files.list) <- 'x'
