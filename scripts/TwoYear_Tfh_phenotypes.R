...
write_tsv( Tfr_vs_GC, 'outs/TwoYear_Tfh_lineages_IL10_TFH_vs_GC_markers.tsv')
Tfr_vs_EM <- FindMarkers(Tfh_obj, ident.1 = Tfr_cells, ident.2 = EM_cells)
Tfr_vs_EM$gene <- rownames(Tfr_vs_EM)
write_tsv( Tfr_vs_EM, 'outs/TwoYear_Tfh_lineages_IL10_TFH_vs_premem_markers.tsv')
EM_vs_GC <- FindMarkers(Tfh_obj, ident.1 =EM_cells, ident.2 = GC_cells) 
EM_vs_GC$gene <- rownames(EM_vs_GC)
write_tsv( EM_vs_GC, 'outs/TwoYear_Tfh_lineages_premem_vs_GC_markers.tsv')

Tfh_obj <- SetIdent(Tfh_obj, value = 'Tfh_type')
Tfh_type_markers <- FindAllMarkers(Tfh_obj)
Tfh_obj <- SetIdent(Tfh_obj, value = 'ident')
write_tsv( Tfh_type_markers, 'outs/TwoYear_Tfh_lineages_Tfh_type_markers.tsv')

Tfh_type_markers_up <- Tfh_type_markers %>%
  filter(avg_log2FC > 0 ) %>%
  group_by(cluster) %>% 
  arrange(cluster, desc(avg_log2FC))
write_tsv( Tfh_type_markers_up, 'outs/TwoYear_Tfh_lineages_Tfh_type_up_markers.tsv')
...
ggsave('outs/TwoYear_Tfh_lineages_Tfh_type_volcano.pdf', vol_plots, 
...
ident_umap_Tfh <- DimPlot(Tfh_obj, pt.size = 2, group.by = "ident" ) + 
...
ggsave('outs/TwoYear_Tfh_lineages_supp_umaps_v1.png', supp_umaps_Tfh1, height = 15, width = 15)

supp_umaps_Tfh2 <- (tissue_umap_Tfh | donor_umap_Tfh) / (TimeUMAP_Tfh2 | year_umap_Tfh) / (picked_clone_umap_Tfh | CloneUMAP_Tfh)

ggsave('outs/TwoYear_Tfh_lineages_supp_umaps_v1.png', supp_umaps_Tfh2, height = 15, width = 15)

#figure umap
figs_umaps_Tfh <- (ident_umap_Tfh | donor_umap_Tfh | plot_spacer() ) / 
  (tissue_umap_Tfh | TimeUMAP_Tfh |  type_umap_Tfh)
figs_umaps_Tfh_v2 <- (ident_umap_Tfh | donor_umap_Tfh | plot_spacer()) / 
  (tissue_umap_Tfh | TimeUMAP_Tfh2 | type_umap_Tfh)
ggsave('outs/TwoYear_Tfh_lineages_figure_umaps.pdf', figs_umaps_Tfh, height = 10, width = 16.5)
ggsave('outs/TwoYear_Tfh_lineages_figure_umaps.png', figs_umaps_Tfh, height = 10, width = 16.5)
ggsave('outs/TwoYear_Tfh_lineages_figure_umaps_v2.pdf', figs_umaps_Tfh_v2, height = 10, width = 16.5)
ggsave('outs/TwoYear_Tfh_lineages_figure_umaps_v2.png', figs_umaps_Tfh_v2, height = 10, width = 16.5)
ggsave('outs/TwoYear_Tfh_lineages_Tfh_type.png',type_umap_Tfh , height = 5, width = 5.5)
...
ggsave('outs/TwoYear_Tfh_lineages_Tfh_type_markers_violin.pdf', 
...
names(pfeat2) <-NULL
p <- DotPlot(Tfh_obj, features = plot_feats, 
             group.by = 'Tfh_type',
...
ggsave('outs/TwoYear_Tfh_lineages_Tfh_type_markers_DotPlot.pdf', 
